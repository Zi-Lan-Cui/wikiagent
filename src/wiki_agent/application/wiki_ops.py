...
class WikiOpsConsumer:
    ...
    async def handle_refine(self, job: Job, progress) -> JobResult:
        ...
        with self._session.open(job) as write:
            ...
            if not summary.files:
                return self._failed(
                    job, write, IngestError(IngestStage.LOAD, "页面加载为空", source=page.name)
                )
            progress("ingest")
            try:
                outcome = await self._pipeline.ingest_one(summary.files[0])
            except IngestError as exc:
                return self._failed(job, write, exc)
            slug = str(page.relative_to(self._wiki_dir)).removesuffix(".md")
            commit = write.commit(commit_subject(Subject.REFINE, slug))
            if outcome.noop:
    ...
    async def handle_restructure(self, job: Job, progress) -> JobResult:
        ...
        with self._session.open(job) as write:
            ...
            subject = commit_subject(
                Subject.RESTRUCTURE, f"{first.op} {'+'.join(first.pages)}"
            )
            if len(proposals) > 1:
    ...
    def _failed(self, job: Job, write: WikiWrite, exc: IngestError | None) -> JobResult:
        """refine 业务失败：abort_export + 事件，不记账（批操作结果非用户待办）。"""
