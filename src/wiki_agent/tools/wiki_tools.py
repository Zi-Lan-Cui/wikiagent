...
class ReadFile(BaseTool):
    ...
    async def _execute(self, file_path: str) -> str:
        ...
        if target is None:
            return (
                f"错误: 路径非法 ({file_path})。"
                f"仅限相对路径且不能含 .. 段，如 concepts/lambda.md"
            )
        try:
            ...
            return f"{file_path} 是目录，请使用 ListDir 查看内容"
        except UnicodeDecodeError:
            return f"无法以 UTF-8 读取: {file_path}"
...
class ListDir(BaseTool):
    ...
    async def _execute(self, dir_path: str, offset: int = 0) -> str:
        ...
        if offset >= len(entries):
            return f"offset={offset} 超出目录项数 {len(entries)}"
        ...
        if has_more:
            lines.append(f"  ... 还有 {len(entries) - offset - len(page)} 项，"
