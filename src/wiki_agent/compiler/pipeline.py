...
from wiki_agent.compiler import Extractor, compile_integrator, refine_integrator
...
class CompilePipeline:
    ...
    def __init__(
        ...
        *,
        ...
        from wiki_agent.compiler.refine import build_index_excluding_self
        ...
        self._save_sources = save_sources if save_sources is not None \
            else (mode != "refine")

        self._converter = Converter(converters=[
            MinerUConverter(backend="pipeline", llm=vlm, caption_images=True,
        ...
        self._extractor = Extractor(
            llm, model_context=model_context,
            max_concurrency=extract_concurrency, wiki_dir=str(self._wiki_dir),
            save_source_page=self._save_sources, prompts=self._prompts,
        )
    ...
    async def _ingest_one(self, raw_file: RawFileProperties) -> IngestOutcome:
        ...
        try:
            ...
            raise IngestError(IngestStage.CONVERT, str(e),
                              source=raw_file.name, cause=e) from e
        except Exception as e:
            raise IngestError(IngestStage.CONVERT, f"未分类: {e}",
                              source=raw_file.name, cause=e) from e
        outcome.converted_chars = len(cf.content)
        logger.info("  [%s] → %d chars, %d 图片",
                    cf.ext, len(cf.content), cf.content.count("!["))
        ...
        # 3. Extract
        sd = SourceDocument(
            name=cf.name, ext=cf.ext, path=str(cf.path),
        ...
        try:
            ...
            raise IngestError(IngestStage.EXTRACT, str(e),
                              source=raw_file.name, cause=e) from e
        except Exception as e:
            raise IngestError(IngestStage.EXTRACT, f"未分类: {e}",
                              source=raw_file.name, cause=e) from e
        ...
        try:
            ...
            raise IngestError(IngestStage.SEARCH, f"未分类: {e}",
                              source=raw_file.name, cause=e) from e
        ...
        try:
            ...
            raise IngestError(IngestStage.ANALYZE, f"未分类: {e}",
                              source=raw_file.name, cause=e) from e
        try:
            # planner 自己知道要不要 current_page（needs_current_page）——
            # pipeline 无条件传，模式知识不泄漏到这里
            outcome.plan = await self._integrator.plan(
                outcome.extract, outcome.analysis,
                schema=schema, purpose=purpose, index_content=index_content,
                current_page=self._current_page(raw_file),
            )
        except IngestError:
            raise
        except Exception as e:
            raise IngestError(IngestStage.PLAN, f"未分类: {e}",
                              source=raw_file.name, cause=e) from e
        ...
        try:
            ...
            raise IngestError(IngestStage.EXECUTE, str(e),
                              source=raw_file.name, cause=e) from e
        except Exception as e:
            raise IngestError(IngestStage.EXECUTE, f"未分类: {e}",
                              source=raw_file.name, cause=e) from e
    ...
    def _append_index(self, plan: IntegrationPlan, pages_written: list[str]) -> None:
        ...
        for pt in plan.page_targets:
            ...
            if f"[[{slug}]]" in existing or slug not in written_slugs:
                continue
            from wiki_agent.compiler.parse import parse_frontmatter
...
def _to_source_chunk(ck, total: int) -> SourceChunk:
    ...
    return SourceChunk(
        content=ck.content, index=ck.chunk_index,
        total=total, heading_path=meta.get("heading_path", ""),
    )
