...
_DEFAULT_MODEL_CONTEXT = 60_000


class Extractor:
    """SourceDocument → ExtractResult。"""

    def __init__(self, llm: LLMClient, *, model_context: int = _DEFAULT_MODEL_CONTEXT, max_concurrency: int = 5, wiki_dir: str | Path | None = None, save_source_page: bool = True, prompts=_compile_prompts):
        self._llm = llm
    ...
    async def extract(self, source: SourceDocument) -> ExtractResult:
        ...
        if uniform:
            logger.info(
                ...
                source.name, len(chunks), budget // len(chunks),
            )
            summaries = await self._extract_uniform(chunks, budget)
        else:
            logger.info(
                "[%s] 滚动压缩, %d chunks", source.name, len(chunks),
            )
    ...
    async def _extract_uniform(
        self, chunks: list[SourceChunk], total_budget: int,
    ) -> list[ChunkSummary]:
    ...
    async def _extract_rolling(
        self, chunks: list[SourceChunk], budget: int,
    ) -> list[ChunkSummary]:
        digest = ""
        ...
        for chunk in chunks:
            response = await async_invoke_with_retry(
                ...
                [
                    ...
                    Message(role="user", content=self._prompts.rolling_user(
                        chunk, digest, len(chunks),
                    )),
                ],
    ...
    def _summary_budget(self) -> int:
        return max(2_000, self._model_context - _SYSTEM_TOKENS - _OUTPUT_TOKENS)
