...
def iter_code_runs(content: str):
...
def check_page_body(content: str) -> tuple[bool, str]:
    ...
    if not body_without_title(content):
...
def check_page_output(content: str) -> tuple[bool, str]:
    ...
    Returns:
        (是否通过, 可执行的错误消息)。
    """
    ok, err = check_page_frontmatter(content)
    if not ok:
        return ok, err
    ok, err = check_page_body(content)
    if not ok:
        return ok, err
    return _check_wikilink_has_text(content)


def check_page_frontmatter(
    content: str,
    *,
