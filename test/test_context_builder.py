"""ContextBuilder build 扩展——稳定 system + 末尾纠错消息。
...
def _build_messages(builder: ContextBuilder) -> list[Message]:
...
def _build(builder: ContextBuilder) -> str:
    return _build_messages(builder)[0].content
...
def test_build_includes_corrections():
    ...
    assert "示例代码有误" in messages[-2].content
