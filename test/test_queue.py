...
def test_append_list_remove_roundtrip():
    ...
    store.append("ingest_failure", source="compile",
                 file="魔法方法.md", stage="plan", error="JSON 错误")
...
def test_queue_command_view():
    ...
    store = QueueStore(tmp)
    store.append("ingest_failure", source="compile", file="x.md",
                 stage="plan", error="JSON 错误")
    ...
    result = asyncio.run(cmd.execute(
        CommandContext(raw="/queue", key="queue", args="",
    ...
    result = asyncio.run(cmd.execute(
        CommandContext(raw="/queue done", key="queue",
                       args=f"done {item_id}",
                       session=session, agent=agent)))
...
def test_queue_command_empty():
    ...
    result = asyncio.run(QueueCommand().execute(
        CommandContext(raw="/queue", key="queue", args="",
